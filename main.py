...
if not 'Config.txt' in os.listdir():
    open('Config.txt', 'w', encoding='utf-8').write('{"Delete_Time":1, "Delete_Time_Shop_Cooldown":15, "guild_id":, "token":""}')
...
async def _stock(ctx=SlashContext):
...
async def _makeshop(ctx=SlashContext, channel_id=str, cooldown=str):
    if not str(ctx.author.id) in ['423747377506025472', '923223323230494790']:
